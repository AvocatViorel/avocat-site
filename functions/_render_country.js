...
function renderBlocks(blocks){
  return (blocks||[]).map(b=>`
    ...
    ${(b.bullets&&b.bullets.length)?`<ul class="list">${b.bullets.map(x=>`<li>${esc(x)}</li>`).join("")}</ul>`:""}
  `).join("");
}
export async function renderCountry(env, lang, slug){
  ...
  return new Response(layout({ title, lang, site, body, path:"/diaspora/"+slug }), { headers: { "content-type":"text/html; charset=utf-8" }});
}
